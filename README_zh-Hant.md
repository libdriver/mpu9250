...
MPU9250是一個多晶片模塊（MCM），由集成到單個QFN封裝中的兩個管芯組成。 一個模具容納3軸陀螺儀和3軸加速度計。 另一個管芯裝有旭化成微器件公司的AK8963 3軸磁強計。 囙此，MPU9250是一種9軸運動跟踪設備，它結合了3軸陀螺儀、3軸加速度計、3軸磁力計和數位運動處理器 ™  （DMP）全部在一個小的3x3x1mm封裝中，可作為MPU6515的引脚相容陞級。 MPU9250通過其專用的I2C感測器匯流排直接提供完整的9軸MotionFusion ™  輸出MPU9250運動跟踪設備及其9軸集成、片上運動融合 ™,  和運行時校準固件，使製造商能够消除離散設備的昂貴和複雜的選擇、鑒定和系統級集成，確保消費者的最佳運動效能。 MPU9250還設計用於在其輔助I2C埠上與多個非慣性數位感測器（如壓力感測器）介面。 MPU925具有三個用於數位化陀螺儀輸出的16比特模數轉換器（ADC）、三個用於對加速度計輸出進行數位化的16比特ADC和三個用於將磁力計輸出數位化的16位數ADC。 對於快速和慢速運動的精確跟踪，該部件具有±250、±500、±1000和±2000°/s（dps）的用戶可程式設計陀螺儀滿刻度範圍、±2g、±4g、±8g和±16g的用戶可編程式加速度計滿刻度範圍以及±4800 μ T的磁強計滿刻度。
...
在線文檔: https://www.libdriver.com/docs/mpu9250/index.html
